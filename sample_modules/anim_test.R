...
nw <- network::network.initialize(50, directed = FALSE)
est <- netest(nw, formation = ~ edges, target.stats = 15,
              coef.diss = dissolution_coefs(~ offset(edges), 10))

## Epidemic model parameterization
param <- param.net(si.prob = 0.75, act.rate = 2, is.rate = 0.25)
init <- init.net(i.num = 10)
control <- control.net(nsteps = 25, nsims = 5, immunity.FUN = immunity,
