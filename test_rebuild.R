...
ergm_formula <- ~ edges + nodematch("pd1") + nodematch("pd2") + nodematch("pd3") +
  ...
  nodematch("pd8") + nodematch("pd9") + edgecov(as.matrix(faux.mesa.high)) +
  edgecov(nocontact_mat)

# TODO: Init Loop
# TODO: Pull target.stats from summary(fmh_sim)
...
target.stats <- attr(fmh_sim, "stats")
...
### 2.5: Network Control
nsteps <- 180
control <- control.net(type = NULL, nsteps = nsteps, nsims = 3, 
